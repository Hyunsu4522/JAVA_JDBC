...
import java.sql.Statement; //ctrl + shift + O
...
public class RunTest {
	...
	public static void main(String[] args) {
		...
		/*
		// 1. 각자 pc(localhost)에 JDBC계정에 연결한 수 TEST테이블에 insert 해보기
		...
		try {
			// 1) jdbc driver등록
			Class.forName("oracle.jdbc.driver.OracleDriver"); //ojdbc6.jar 파일을 추가 안했을경우 | 추가는 했는데 오타가 있을 경우 => ClassNotFoundException발생
			...
			//4, 5) sql문 전달하면서 실행 수 결과받기(처리된 행수)
		...
		*/
		...
		// 2. 내 pc DB상에 jdbc TEST테이블에 모든 데이터 조회해보기
		...
		try {
			...
			while(rset.next()) {
				// 현재 참조하는 rset으로 부터 어떤 컬럼에 해당하는 값을 어떤타입으로 뽑을건지 제시해줘야한다.
				// db에 컬럼명을 제시(대소문자를 가리지 않는다.)
